...
const UnReportedPosts = () => {
    ...
    const getPosts = async (page = postsPage) => {
        ...
        await axios.get(`${backend}/posts/reported/posts?page=${page}`, {headers: {Authorization: `Bearer ${token}`}})
            .then((response) => {
        ...
        <Grid container className p={2}>
            <Grid p={3} pt={0} xs={12} sx={{
                ...
                <Grid item p={2} m={1} xs={12} sm={12} md={12} container direction="row" justifyContent="space-between"
                      alignItems="center">
                    <Typography color="#343434" sx={{fontSize: '26px', fontWeight: '500'}}>
                ...
                {
                    posts && posts.map((post, key) => (<ContentModerationVideoItem key={key} post={post}/>))
                }
